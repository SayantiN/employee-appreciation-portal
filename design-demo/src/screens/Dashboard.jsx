...
export function Dashboard() {
  ...
  // A votable cycle beats a draft that merely exists — otherwise a December
  // draft would announce itself as open all year.
  const pick = (type) => data.openCycles.find((c) => c.award_type === type && c.votable)
    || data.openCycles.find((c) => c.award_type === type);
  const month = pick('Month');
  const year = pick('Year');
  const primary = month?.votable || !year?.votable ? month || year : year;
